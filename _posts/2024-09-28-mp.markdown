...
这里首先是需要一定时间去进行构思，首先是Gui界面，这方面我只是稍微看了一些常用的API，独立开发一个就算很简单但是美观的界面也需要一段时间，这里选择用通义千问来帮我开发Gui框架，说实在通义的代码逻辑很好，有着很好的扩展性，有必要的提示让我去补充剩下的功能代码，多提几次要求，一个较为美观，可以随意切换界面的GUI界面就完成了


注册和登录界面过于简单就不展示了，由于现在对于mysql仅仅是刚刚入门，掌握了一部分基础操作，还不足以让我实现联合编程，这里还是使用原始的服务端利用txt文档来存储账号密码数据，启动后不断自动同步读取进入内存，方便对客户端发来的登录和注册请求进行判断，
```java
 // 登录按钮监听器
